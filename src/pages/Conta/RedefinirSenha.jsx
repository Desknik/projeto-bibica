...
export default function Perfil(userData) {
  ...
  const criarToken = async () => {
    ...
    {email}
      ...
      if(response.ok){
        Router.push("/Conta/AlterarSenha")
      }else{
  ...
  return (
    <>
      ...
      <div style={{height: '70vh'}} className="flex justify-center items-center p-8">
        ...
        <span ref={errorBoxRef} className='absolute bottom-5 right-5 z-30  opacity-0 transition-opacity duration-300 ease-in-out bg-red-400/60 border-2 border-red-500 text-red-200 font-medium rounded-lg inline-block px-2 py-2 my-1'>{errorMessage}</span>
          ...
          <div className="flex flex-col items-center gap-3">
            <h1 className='font-bold text-xl mb-5'>Alterar minha senha</h1>
            <p>Informe seu email e receba as informações para alterar sua senha.</p>
            ...
            <button disabled={requisição} onClick={() => criarToken()} className='w-full px-2 py-1.5 bg-pink-400 rounded-md text-white hover:bg-pink-500'>Redefinir Senha</button>
